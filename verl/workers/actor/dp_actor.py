...
class DataParallelPPOActor(BasePPOActor):
    def __init__(self, config, actor_module: nn.Module = None, actor_optimizer: torch.optim.Optimizer = None, inference_only=False, tokenizer=None, processor=None):
        ...
        # Note(vermouth1992): we move the device mesh constructor into the DataParallelActor
        # so that DataParallelActor can be instantiated solely base on config. We keep compatible with old approach
        # that constructing model and optimizer, when pass them as arguments. We will remove the compatibility in v0.6
        ...
        if self.actor_module is None:
            ...
            assert self.actor_optimizer is None
    ...
    def _build_model_optimizer(
        ...
        if torch_dtype is None:
            torch_dtype = torch.float32 if not self.inference_only else torch.bfloat16
        else:
        ...
        if fsdp_strategy == "fsdp":
            ...
            if not self.inference_only and fsdp_config.offload_policy:
                cpu_offload = CPUOffloadPolicy(pin_memory=True)
            else:
                cpu_offload = None if not self.inference_only else CPUOffloadPolicy(pin_memory=True)
        ...
        log_gpu_memory_usage("After FSDP init", logger=logger)

        # TODO: add more optimizer args into config
        if not self.inference_only and optim_config is not None:
    ...
    def to(self, tag, device):
        assert tag in ["model", "optimizer"]
        if tag == "model":
            if device == "cpu":
